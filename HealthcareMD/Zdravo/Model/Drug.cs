...
namespace Model
{
    public class Drug
    {
        ...
        private HealthcareMD.Status status;
        public HealthcareMD.Status Status { get { return status; } set { status = value;} }
            ...
            get {
                ...
                switch (status)
                {
                    ...
                    case (HealthcareMD.Status.denied):
                        retVal = "ODBIJENO";
                        break;
                    case (HealthcareMD.Status.reported):
                        retVal = "PRIJAVLJENO";
                        break;
                    default:
                        retVal = "NA ČEKANJU";
        ...
        public Drug(int id, string name, HealthcareMD.Status status, string type, string description, List<string> ingredients, int alternativeDrugId)
        {
            this.id = id;
            this.name = name;
            this.status = status;
        ...
        internal void FromCSV(GroupCollection csvValues)
        {
            ...
            {
                case "A":
                    status = HealthcareMD.Status.accepted;
                    break;
                case "D":
                    status = HealthcareMD.Status.denied;
                    break;
                case "R":
                    status=HealthcareMD.Status.reported;
                    break;
                default:
                    status = HealthcareMD.Status.waiting;
                    break;
            }
